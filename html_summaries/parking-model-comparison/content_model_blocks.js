...
output = {
    ...
    <table class="compare dense aligned">
      ...
      <tr><td>Latent action and behavior control at inference</td><td>In current SI parking, <code>enable_latent_action=True</code> because <code>w_latent_action=1.0</code>, while <code>enable_behavior_control=False</code>. During training, the latent id comes from privileged GT policy waypoints. During inference, there are no GT waypoints, so <code>LatentActionModule</code> predicts logits, takes argmax, looks up the learned latent-action codebook vector, and adds that token to every context token before final output queries decode waypoints/indicator/gear. No behavior token is added in this config.</td><td>Config ${link(gh.cur, "wayve/ai/si/configs/parking/parking_config.py", 210, "w_latent_action=1")}; train/inference branch ${link(gh.cur, "wayve/ai/zoo/outputs/output_adaptor.py", 465, "latent conditioning")}; argmax/codebook ${link(gh.cur, "wayve/ai/zoo/outputs/latent_action_module.py", 48, "latent-action inference")}.</td></tr>
    </table>
