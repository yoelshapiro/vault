...
model.load_state_dict(remapped_state_dict, strict=False)</code></pre>
    ...
    <table class="compare dense aligned">
      ...
      <tr><td>What does WTA mean here?</td><td><b>Winner-Take-All</b> means the training code compares every predicted head against the same observed future, computes one route score per head, and gives the largest loss weight to the best-matching head. Zak uses an annealed soft version rather than a hard argmin for the whole run: high temperature early spreads gradient across heads; low temperature late makes the target nearly one-hot.</td><td>${link(gh.zak, "wayve/ai/experimental/losses_metrics/common.py", 1816, "AnnealedWTALoss")}; ${link(gh.zak, "wayve/ai/experimental/losses_metrics/common.py", 1858, "temperature schedule")}.</td></tr>
      <tr><td>How can each head get a loss with one trajectory?</td><td>There is only one demonstrated future per sample/frame. The code does <b>not</b> need eight labels. It broadcasts the same GT trajectory, indicator, and gear targets across the eight predictions, computes <code>ego_loss[k]</code>, <code>indicator_loss[k]</code>, and <code>gear_loss[k]</code> for every head <code>k</code>, then uses the detached per-head route score to form <code>wta_weight[k]</code>. The final supervised loss is a weighted sum over those eight per-head losses.</td><td>${link(gh.zak, "wayve/ai/experimental/losses_metrics/common.py", 2050, "per-head losses")}; ${link(gh.zak, "wayve/ai/experimental/losses_metrics/common.py", 2144, "route score")}; ${link(gh.zak, "wayve/ai/experimental/losses_metrics/common.py", 2155, "soft WTA weights")}.</td></tr>
      <tr><td>Is this MoE or an ensemble?</td><td>It is best described as <b>multi-head WTA / multiple-hypothesis prediction with a learned mode classifier</b>. It is MoE-like because there are expert heads and an inference-time gate. It is not a normal ensemble because the heads share one backbone, train jointly, and are not averaged/voted. It is also not a classical MoE layer because the classifier does not route training gradients; the WTA loss oracle computes the training assignment from supervised per-head errors, and the classifier learns to imitate that assignment.</td><td>Heads ${link(gh.zak, "wayve/ai/experimental/models/mcv_perceiver.py", 3248, "head banks")}; classifier target ${link(gh.zak, "wayve/ai/experimental/losses_metrics/common.py", 2201, "soft classifier loss")}.</td></tr>
...
# One sample has one observed future. Every head is compared to that same label.
gt_ego = inputs["egoposition"]                        # [B,T,n,2]
gt_indicator = inputs["indicator"]                    # [B,T,n]
gt_gear = inputs["gear"]                              # [B,T,n]
for k in range(8):
    ego_loss[k] = regression_loss(all_ego[k], gt_ego)
    indicator_loss[k] = cross_entropy(all_ind[k], gt_indicator)
    gear_loss[k] = cross_entropy(all_gear[k], gt_gear)
