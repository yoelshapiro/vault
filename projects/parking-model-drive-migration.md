...
| 3 | clamp at first neutral `clamp_policy_at_first_neutral` ([parking.py:845](wayve/ai/si/datamodules/parking.py)) | `clamp_policy_at_neutral` (tag 12) — ✅ **DONE** (`5acbd018`; schema 7.8.0) | Pipe freezes the policy trajectory from the first neutral (park) step: repeat pose/waypoints/curvature, force rest-of-gear to N, zero speed from there. Bucket+stage-gated (PARKING/UNPARKING); runs after `gear_label_cleanup`; `standstill_gear` now orders after it (clamped loss-step target). | policy_pose, policy_waypoints, policy_speed, policy_gear_direction (req); policy_curvature (optional); `read_only_fields=[parking_stage]` |
| 4 | strip leading standstill `strip_leading_standstill` ([parking.py:697](wayve/ai/si/datamodules/parking.py)) | `strip_leading_standstill` (tag 13) — ❌ **DROPPED (user decision)** — not migrating. | — | — |
