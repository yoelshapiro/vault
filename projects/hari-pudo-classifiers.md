...
- Correct new-HARI upload path from the Notion HARI Upload Guide: use `/blob/<storage-account>/<container>/<path>` file keys with `HARIUploader(use_file_key_mode=True)`, not the old SAS-backed `external_media_source` flow in `hari_upload_videos upload`.
