...
- Changes:
  - [[agent_tasks/2026/05/Week-4/2026-05-21-fuchsia-velociraptor-parking-deploy]]: resolved source session `session_2026_05_20_17_05_40_si_parking_bc_train_release_2026_5_11_unpudo_moving_speed_intervention` and latest checkpoint `10` / step `100000`, confirmed no source radar overlay was needed, spawned the required deploy sub-agent, and recorded the failed deploy because the current branch's `deploy.py` lacks `--enable_interleave_control` / `--interleave_control_group`; identified `03-20-si-group-interleave-control-support` as a branch containing the needed deploy flags and resolved the requested Eval Studio suite history to canonical suite UUID `ea663952-b914-47a3-8cc1-729db3683dce`.
  - Retried the requested interleave-control deploy from detached worktree `/tmp/wayvecode-fuchsia-model-branch` at training commit `1beb40334ef9cbaf8ac17a45299066570755171c`; the deploy sub-agent reached the deploy binary but failed before output-session creation due `DefaultAzureCredential` failing to get an Azure token for `wayveprodmlexperiments.blob.core.windows.net/training-session-store`.
  - Completed the interleave-control deploy using a symlinked source-session overlay to avoid the older deploy code's AzureFileSystem credential path; created deployed model `circumspect-harlequin-elephant`, session `session_2026_05_20_17_05_40_si_parking_bc_train_release_2026_5_11_unpudo_moving_speed_intervention__fuchsia-multicolored-velociraptor_interleave_control_v2`, checkpoint `1`, gen2 artefact `636d5bca-3184-4b44-b8a8-b6820ac18f53`, and verified parking radar plus `INTERLEAVE_GROUP_PARKING`.
  - Triggered supported Alpha 3 Model CI build `72522` for the deployed gen2 artefact, with archive and Eval Studio passed, HiL running, and license blocked behind the pipeline gate; noted the API does not expose a literal Gen3 Model CI target.
  - Triggered the requested `Pudo-Unpudo` Eval Studio suite execution `1d643e3a-c123-4f1e-a434-2b2b52423bd3` against artefact `636d5bca-3184-4b44-b8a8-b6820ac18f53`; last observed status was `IN_PROGRESS` with `2130` cases in progress.
  - Attempted to add the standard parking `model_change_note`, but local Console SSO redirected to OneLogin and the direct Model Catalogue write returned `401 User authorisation required`.
...
- Changes:
  ...
  - Re-read the PR 106346 diffusion implementation and corrected Wonjoon's graph/content: target labels are now shown as training-only flows rather than runtime adaptors, inference-only generated-path flow is explicit, the Wonjoon tab has detailed diffusion-head mechanics plus train/inference pseudo-code, and the page calls out that PR 106346 does not wire a separate goal-pose input adaptor despite the Notion design discussing optional goal-pose conditioning.
  - Reworked the Wonjoon Full Architecture graph to reduce line overlap: runtime inputs now merge through a token bus, and training targets/losses were removed from the architecture graph because they are not model modules.
...
- Areas: `agent_tasks/2026/05/Week-3/`
- Changes:
  - [[agent_tasks/2026/05/Week-3/2026-05-13-loquacious-partridge-interleave-deploy]]: started deploy with suffix `__loquacious-partridge-lime_interleave_control_v2` from the source session directly after confirming no source `full_config.yml` radar overlay keys were present; later overlay retries removed stale `include_tele_lens_blockage`, set WFM pretraining loader `strict: false`, and disabled Model CI, but the final retry failed during trained checkpoint load because indicator-head checkpoint weights have 4 classes while the current model instantiated 3 classes.
...
- Changes:
  ...
  - Investigated failure of `157814` as likely infra/startup master rendezvous failure, then restored it as retry job `157951` (`butterfly-fuchsia-outgoing-157951`), observed `Dispatched`.
...
- Changes:
  - [[agent_tasks/2026/04/Week-5/2026-04-30-parking-training-wonjoon-full-aug]]: recorded the exact training command and prompt answers, captured Surfboard job `156658`, session `session_2026_04_30_19_28_13_si_parking_bc_train_gear_indicator_wonjoon_full_aug`, nickname `violet-happy-dolphin`, W&B and Datadog links, and confirmed the job reached `Running` on `aks-prod-training-2-swe.nd96h100c` without creating or updating a Notion release row.
...
- Areas: `agent_tasks/2026/04/Week-5/`
- Changes:
  - [[agent_tasks/2026/04/Week-5/2026-04-30-parking-training-gi-parking-aug-retry]]: recorded the exact training command and prompt answers, captured Surfboard job `156652`, session `session_2026_04_30_19_00_22_si_parking_bc_train_gear_indicator_gi_parking_aug_retry`, nickname `pink-owl-vociferous`, W&B and Datadog links, and confirmed the job reached `Running` on `aks-prod-training-2-swe.nd96h100c` without creating or updating a Notion release row.
...
- Topic: Parking model comparison report
  ...
  - Changes:
    ...
    - Added solution-specific Latents & Multimodal diagrams and pseudo-code-heavy deep dives for the major technical paths.
    ...
    - Added an explicit WTA clarification to the HTML: Zak compares every head to the one observed future in the sample, uses the per-head losses to form annealed WTA weights, trains the classifier to imitate that WTA assignment, and is best described as multi-hypothesis/MoE-like rather than an ensemble.
...
- Topic: Parking route-end input options
  ...
  - Changes:
    ...
    - Updated `ParkingDataConfig` defaults to use Guy branch's active BC parking values where supported by this branch.
...
- Topic: PR 102690 merge blocker fixes
  - Labels: parking, datamodule, route-map, metrics, tests
  - Branch: boris/03-23-park-route-shortening-v2
  - PR: 102690
  - Change type: Bug Fix / Test Repair
  - Areas: wayve/ai/lib/data/pipes, wayve/ai/si/datamodules, wayve/ai/si/metrics
  - Changes:
