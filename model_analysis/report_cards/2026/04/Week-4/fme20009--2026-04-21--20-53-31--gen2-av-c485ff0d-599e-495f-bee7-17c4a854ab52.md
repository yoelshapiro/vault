...
- Not scored: this is not a valid AV-owned UNPUDO segment. The local controller window is DBW-false at the anchor, first motion happens before AV re-engages, and the first recovered AV engagement arrives only `2.318s` after the event start. This event can describe the run history, but it should not be used as model pass/fail evidence.
...
The important fact here is ownership, not outcome. The car is already moving before the locally recovered AV re-engagement, so the success belongs to the pre-AV or handover portion rather than to the model-controlled segment. Because only the final `2.882s` overlap AV ownership, this should be treated as a handover artifact, not a model success or failure.
...
- Not scored: the event completes while AV is locally DBW-false, and the first recovered AV re-engagement comes `2.238s` after the official event end. The run-level outcome is successful, but this particular anchor is not AV-owned evidence for the model.
...
- Not scored: the official event starts and ends while the local controller window is DBW-false, and AV does not re-engage until `4.317s` after the event has already ended. This is a manual or handover-completed success, not model-owned evidence.
...
This is the clearest example of why the baseline matters. The vehicle completes the event before AV ownership returns, so the successful rollout says nothing useful about model quality. It should stay in the report for completeness, but it should be excluded from success-rate reasoning.
