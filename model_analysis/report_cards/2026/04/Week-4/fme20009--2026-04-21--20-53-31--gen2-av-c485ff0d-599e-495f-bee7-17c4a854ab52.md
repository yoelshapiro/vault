...
- Pass: successful UNPUDO with an ownership-boundary caveat. Planned and actual gear are both `DRIVE`, the vehicle begins moving `0.137s` after the anchor with no accelerator help, and the event closes without disengagement. The nearest trajectory sample at the anchor has `is_drive_by_wire = false`, and the actual indicator at the anchor is `OFF` despite a planned `RIGHT_ON`, so the exact AV/manual boundary should be treated as uncertain rather than cleanly AV-owned.
...
This segment finishes cleanly and shows the model selecting and holding `DRIVE` through the maneuver window. The car starts moving almost immediately after the event anchor, reaches a moderate rollout speed, and never needs driver accelerator input. The pass is still caveated because the nearest trajectory sample at the anchor is not DBW-active and the actual indicator sample is `OFF` while the plan wants `RIGHT_ON`; that looks more like a handover-boundary artifact than a failed maneuver.
