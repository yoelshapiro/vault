...
- [x] (2026-06-15) Added synced telemetry charts (user request): speed, av mode, gear, indicator. New `sources/signals.py` + `GET /api/signals` query `prod_data_pipeline.wayve_corpus.all_data` (`inferred__state__odometry__speed_kmh`, `ground_truth__state__vehicle__automation_active`/`gear_direction`/`indicator_light`) by `run_id`+`timestamp_unixus` over the clip window, cached on disk. New `static/charts.js` renders 4 SVG panels sharing the clip's time axis with a white playhead driven by the player's rAF loop (`onLoad`/`onPlayhead` hooks + `_clipWindow`), an event green band, and per-panel value readouts. Verified live: `/api/signals` returns 81 aligned samples for the example event; `py_test` (26) + lint + ty + eslint green; charts.js served on :3006.
- [x] (2026-06-15) Second browser round — 3 fixes: (1) duplicate charts were a stale-async race (a slow `/api/signals` for a previous event resolving after navigation appended panels) — added a render `seq` guard + clear-before-draw in `static/charts.js`; (2) autoplay skipping — added retry-once on the first (transient cold-cut) video error before counting it no-media; (3) slow telemetry — root-caused to corpus point-lookup cost (~13s cold, same for 1 vs 5 partitions; no run_id clustering on `all_data`), added `run_date_iso` partition pruning (lead with the run-id-embedded date), telemetry prefetch (1-2 ahead, concurrency-capped) in `app.js`, and confirmed disk cache makes re-views/prefetched events instant (13.0s cold -> 0.006s cached). 29 tests + lint + ty + eslint green.
- [ ] DECISION for user: ~13s first-view per run-day is inherent to `prod_data_pipeline.wayve_corpus.all_data`. Faster options if needed: (a) investigate run-keyed raw stream tables (robot_vehicle_driver_vehicle_state + robot_control_controller_state) — only helps if clustered by run_id; (b) bigger/serverless warehouse (infra change, needs approval); (c) pre-materialise telemetry for the event set. Awaiting user choice.
- [ ] User re-test (hard-refresh) — confirm no duplicate charts, browsing telemetry is warm, autoplay skips less.
