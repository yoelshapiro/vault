...
- Added explicit WTA/MoE/ensemble clarification: WTA means Winner-Take-All, each Zak head is compared to the single observed future from the sample, per-head supervised errors create the annealed WTA target, and the approach is multi-hypothesis/MoE-like rather than an ensemble.
...
- Re-read Wonjoon's `parking_config.py`, `DiffusionOutputAdaptor`, `DiffusionHead`, `PathPosePrePostProcessor`, `PolicyPathConditioner`, and `OrdinaryHead` implementations and corrected the graph semantics: training labels are not runtime input-adaptor tokens; they are now shown as dashed training-only target/loss flows.
- Reworked the Wonjoon Diffusion tab with a fuller solution explanation, diffusion-head mechanics, path encoder/decoder details, explicit train-vs-inference pseudo Python matching `DiffusionOutputAdaptor._train()` / `robot_inference()`, and a clarification that the Notion goal-pose idea is not wired as a separate goal-pose adaptor in PR 106346.
- Reworked the Wonjoon full-architecture graph for readability after line-overlap feedback: runtime input fan-in now uses a token bus.
- Removed training targets, target processors, and training losses from the Wonjoon Full Architecture graph after review; those are training/evaluation concerns, not NN module architecture, and remain documented in the Wonjoon Diffusion tab.
...
- Verified the revised Wonjoon content: all JS passes `node --check`; the full-architecture section still has three SVG graphs; the Wonjoon graph includes training-only and inference-only edge classes; the Wonjoon tab includes the diffusion-head mechanics and train/inference pseudo-code.
- Verified the simplified Wonjoon full-architecture graph registers with the `2400x1160` viewBox, token bus, and no `Training losses` / `Target processors` nodes; `content_full_arch_wonjoon.js` remains under 500 lines.
- Verified the WTA clarification is served from port 3005 in `content_model_blocks.js`, `content_latents.js`, and `content_core.js`; `node --check` passes for the edited scripts and all edited report JS files remain under 500 lines.
