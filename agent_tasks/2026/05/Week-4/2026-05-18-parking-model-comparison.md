...
- Reworked the Wonjoon full-architecture graph for readability after line-overlap feedback: runtime input fan-in now uses a token bus, and training targets/losses are separated into a bottom lane instead of crossing through the runtime model path.
...
- Verified the simplified Wonjoon full-architecture graph registers with the larger `2400x1320` viewBox, token bus, and separate training-target lane; `content_full_arch_wonjoon.js` remains under 500 lines.
